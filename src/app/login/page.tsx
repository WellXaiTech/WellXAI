...
function LoginPageInner() {
  ...
  const ssoErrorFromRedirect = searchParams.get("ssoError");

  async function handleSsoContinue() {
    if (ssoBusy || !ssoEmail.trim()) return;
    setSsoBusy(true);
    setSsoError(null);
    try {
      ...
      if (!res.ok) throw new Error(data.error || "No SSO connection found for that email's domain");
      window.location.href = data.redirectUrl;
    } catch (err) {
      setSsoError(err instanceof Error ? err.message : "Couldn't start SSO sign-in");
      setSsoBusy(false);
    }
  }
        ...
        <div className="mt-6 flex flex-col gap-2.5 md:mt-10 md:gap-3">
          ...
          {!ssoOpen ? (
            ...
            <div className="rounded-2xl border border-border bg-surface p-3">
              <label className="mb-1.5 block text-xs text-muted">Work email</label>
              <input
                value={ssoEmail}
                onChange={(e) => setSsoEmail(e.target.value)}
                onKeyDown={(e) => {
                  if (e.key === "Enter") handleSsoContinue();
                }}
              ...
              />
              {ssoError && <p className="mt-2 text-xs text-red-500">{ssoError}</p>}
              <button
                onClick={handleSsoContinue}
                disabled={ssoBusy || !ssoEmail.trim()}
                className="mt-2.5 w-full rounded-full bg-foreground px-4 py-2 text-sm font-semibold text-background disabled:opacity-40"
              >
